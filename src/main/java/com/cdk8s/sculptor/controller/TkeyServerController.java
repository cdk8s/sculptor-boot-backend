...
public class TkeyServerController {
	...
	@Autowired
	private StringRedisService<String, Boolean> loginUsernameValidateCodeRedisService;
	...
	public String formLogin(final HttpServletRequest request, final HttpServletResponse response, ModelMap model, OauthFormLoginParam oauthFormLoginParam) {
		...
		try {
			...
			// 校验验证码
			if (needImageValidateCode(oauthFormLoginParam.getUsername())) {
			...
			if (e instanceof ValidateCodeException || needImageValidateCode(oauthFormLoginParam.getUsername())) {
				model.put("deviceId", GenerateIdUtil.getUUID());
			}
	...
	private Boolean needImageValidateCode(String username) {
		...
		Boolean flag = loginUsernameValidateCodeRedisService.get(GlobalConstant.REDIS_LOGIN_USERNAME_VALIDATE_CODE_KEY_PREFIX + username);
		if (null != flag && flag) {
			// 根据 redis 中该用户是否有需要验证验证码标识进行判断
			return true;
		}
		...
		serviceBO.setBoolLoginSuccessEnum(BooleanEnum.NO.getCode());
		serviceBO.setUsername(username);
		List<SysLoginLog> listByServiceBO = sysLoginLogService.findListByServiceBO(serviceBO);

		if (CollectionUtil.isNotEmpty(listByServiceBO) && listByServiceBO.size() > loginErrorNumber) {
			// 如果该用户满足限制次数，则返回 deviceId 设备号，同时存储到 redis 中一个标识，标识该用户今天的所有登录都必须需要验证码。redis 有效期为今晚 24 时减去当前时间得到的秒数
			loginUsernameValidateCodeRedisService.set(GlobalConstant.REDIS_LOGIN_USERNAME_VALIDATE_CODE_KEY_PREFIX + username, true, DatetimeUtil.betweenNowAndFutureTime(DatetimeUtil.getLocalDateTimeEnd(DatetimeUtil.currentEpochMilli()), ChronoUnit.SECONDS));
			return true;
		}
