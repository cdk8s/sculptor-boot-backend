...
public class OauthSaveService {
	...
	@Async
	public void saveCodeToRedis(String code, String tgc, String userInfoRedisKey, String clientId, OauthRequestInfoBO oauthRequestInfoBO) {
		...
		codeRedisService.set(GlobalConstant.REDIS_OAUTH_CODE_PREFIX_KEY_PREFIX + code, oauthCodeToRedisBO, oauthProperties.getCodeMaxTimeToLiveInSeconds(), TimeUnit.SECONDS);
	}
	...
	public void saveTgcToRedisAndCookie(String tgc, Integer maxTimeToLiveInSeconds, String userInfoRedisKey, String userAgent, String requestIp, boolean isRememberMe) {
		...
		oauthTgcToRedisBO.setIat(DatetimeUtil.currentEpochSecond());
		tgcRedisService.set(GlobalConstant.REDIS_TGC_KEY_PREFIX + tgc, oauthTgcToRedisBO, maxTimeToLiveInSeconds, TimeUnit.SECONDS);
	}

	@Async
	public void updateTgcAndUserInfoRedisKeyExpire(String tgc, String userInfoRedisKey) {
		tgcRedisService.expire(GlobalConstant.REDIS_TGC_KEY_PREFIX + tgc, oauthProperties.getTgcAndUserInfoMaxTimeToLiveInSeconds(), TimeUnit.SECONDS);
	...
	private void saveAccessTokenToRedis(String accessToken, OauthAccessTokenToRedisBO oauthAccessTokenToRedisBO) {
		accessTokenRedisService.set(GlobalConstant.REDIS_OAUTH_ACCESS_TOKEN_KEY_PREFIX + accessToken, oauthAccessTokenToRedisBO, oauthProperties.getAccessTokenMaxTimeToLiveInSeconds(), TimeUnit.SECONDS);
	}

	private void saveRefreshTokenToRedis(String refreshToken, OauthRefreshTokenToRedisBO oauthRefreshTokenToRedisBO) {
		refreshTokenRedisService.set(GlobalConstant.REDIS_OAUTH_REFRESH_TOKEN_KEY_PREFIX + refreshToken, oauthRefreshTokenToRedisBO, oauthProperties.getRefreshTokenMaxTimeToLiveInSeconds(), TimeUnit.SECONDS);
	}
